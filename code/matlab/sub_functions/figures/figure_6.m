%% figure_6.m
% Sub-function of Irish_Tuna.m; plots habitat envelopes per hotspot
...
for j = 1:6
    ...
    [binned.N,binned.Temp,binned.Depth,binned.binT,binned.binD] = histcounts2([pdt.MinTemp(pdt.Region == j-1); ...
        pdt.MeanTemp(pdt.Region == j-1); ...
        pdt.MaxTemp(pdt.Region == j-1)], ...
        [pdt.Depth(pdt.Region == j-1); ...
        pdt.Depth(pdt.Region == j-1); ...
        pdt.Depth(pdt.Region == j-1)],binned.Temp,binned.Depth);
    binned.N(binned.N == 0) = NaN;

    N = length(unique(pdt.toppID(pdt.Region == j-1)));
    ...
    cd([fdir 'figures']);
    exportgraphics(gcf,['figure_6_' rg '.png'],'Resolution',300);
